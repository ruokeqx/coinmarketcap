...
<div>
  <el-row class = 'row'  style="width:80%;text-algin:center; margin-left: 10%; margin-top: 10px">
    <el-col :span="12"><div class="grid-content bg-purple" style="font-weight: bolder;
      font-size: 18px;">Collection Cryptocurrency </div></el-col>
  </el-row>
  <el-table :data="tableData"  style="width:80%;text-algin:center; margin-left: 10%; font-size: 15px" :header-cell-style="{color: '#000000'}" :cell-style="{color: '#000000'}" :row-style="{height:70+'px'}" >
    ...
    >
    <template slot-scope="scope">
        <el-button @click="handleClick(scope.row)" type="danger" icon="el-icon-star-off" circle ></el-button>
      </template>
    </el-table-column>
    <!-- <el-table-column
      prop="Name"
...
export default {
  ...
  methods: {
    ...
    async getData () {
      await this.$http.get('data-api/v3/cryptocurrency/like',{
        headers: {
        ...
        if (res.status === 200) {
            for(var i=0;i<res.data.length;i++){
              ...
              this.$http.get('data-api/v3/cryptocurrency/historical?coinName=' + res.data[i].Coinname + '&timeStart=1630936389&timeEnd=1630936389', {
                headers: {
                'token':  window.sessionStorage['token']
                }
              }).then(res1 => {
                ...
                if (res1.status === 200) {
                  ...
                  this.tableData.push({ Index: this.index, Name: res1.data[0].Name, Id: res1.data[0].Id, Price: '$' + res1.data[0].HighPrice.toLocaleString(), Volume: '$' + parseInt(res1.data[0].Volume).toLocaleString(), MarketCap: '$' + parseInt(res1.data[0].MarketCap).toLocaleString(), d7: 0 + '%', h24: 0 + '%' })
                  this.params.push(res1.data[0].Id)
                }
        })
    ...
    handleClick(row){
      ...
      console.log(row.Name)
       this.$http.delete('data-api/v3/cryptocurrency/like?coinName='+row.Name, {
          headers: {
            'token':  window.sessionStorage['token']
          }
        }).then(function(data){
