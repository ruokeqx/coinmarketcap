...
<div>
  <h1>详细数据</h1>
  <el-table :data="tableData"  style="text-algin:center; font-size: 15px" :header-cell-style="{color: '#000000'}" :cell-style="{color: '#000000'}" :row-style="{height:60+'px'}" >
     <el-table-column
      prop="Index"
      width="70"
      align="center"
...
export default {
  ...
  methods: {
    ...
    async getData () {
        await this.$http.get('data-api/v3/cryptocurrency/historical_page_num?coinName=' + this.coin , {
          headers: {
        ...
        await this.$http.get('data-api/v3/cryptocurrency/historical_page?coinName=' + this.coin + '&pages=' + this.page + '&limits=' + this.pageSize , {
          headers: {
            'token':  window.sessionStorage['token']
          }
        }).then(res => {
